...
function renderPlanCard(plan, options = {}) {
  ...
  const editHref = options.organizationId ? `/org-ai-campaign-operator?organization_id=${Number(options.organizationId)}&edit_plan=${Number(plan.id)}` : `/admin/ai-campaign-operator?edit_plan=${Number(plan.id)}`;
...
function renderOperatorPage({ role, organizationId, placements = [], plans = [], editPlan = null, error = "" }) {
  ...
  const brief = editPlan ? (typeof editPlan.brief_json === "string" ? JSON.parse(editPlan.brief_json) : (editPlan.brief_json || {})) : {};
  ...
  <section style="margin-top:20px;border:1px solid ${editPlan?'#2563eb':'#dbe4f0'};border-radius:16px;padding:20px;background:#fff;"><h2 style="margin-top:0;color:#102b50;">${editPlan?'Edit recommended plan':'Campaign brief'}</h2>${editPlan?'<p style="color:#52667e;">Update the recommendation inputs below. Vivid will recalculate the warning, evidence and performance targets before returning it for approval.</p>':''}<form method="POST" action="${base}${query}">${editPlan?`<input type="hidden" name="plan_id" value="${Number(editPlan.id)}">`:''}<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:14px;"><div><label>Campaign name</label><input name="name" required placeholder="Friday Night Family Offer" value="${escapeHtml(brief.name||'')}"></div><div><label>Objective</label><select name="objective"><option value="engagement"${selected(brief.objective,'engagement')}>Increase clicks and engagement</option><option value="conversion"${selected(brief.objective,'conversion')}>Generate conversions</option><option value="awareness"${selected(brief.objective,'awareness')}>Build awareness</option><option value="inventory"${selected(brief.objective,'inventory')}>Promote available inventory</option></select></div><div><label>Audience</label><input name="audience" required placeholder="Families attending home football games" value="${escapeHtml(brief.audience||'')}"></div><div><label>Offer or message</label><input name="offer" required placeholder="Show this page for a family-night offer" value="${escapeHtml(brief.offer||'')}"></div><div><label>Placement</label><select name="placement_id" required><option value="">Choose placement</option>${placements.map(item=>`<option value="${Number(item.id)}"${selected(brief.placementId,item.id)}>${escapeHtml(item.name)}</option>`).join("")}</select></div><div><label>Budget</label><input name="budget" type="number" min="0" step="0.01" value="${Number(brief.budget||0)}"></div><div><label>Start date</label><input name="start_date" type="date" required value="${escapeHtml(brief.startDate||'')}"></div><div><label>End date</label><input name="end_date" type="date" required value="${escapeHtml(brief.endDate||'')}"></div><div style="grid-column:1/-1;"><label>Events, timing or instructions</label><input name="event_notes" placeholder="Home games, Friday evenings, or other constraints" value="${escapeHtml(brief.eventNotes||'')}"></div></div><div style="display:flex;gap:9px;align-items:center;flex-wrap:wrap;margin-top:16px;"><button type="submit" style="border:0;border-radius:10px;background:#2563eb;color:#fff;padding:12px 16px;font-weight:900;cursor:pointer;">${editPlan?'Save & Recalculate Plan':'Prepare Campaign Plan'}</button>${editPlan?`<a href="${approvalHref}" style="color:#173b6b;font-weight:800;text-decoration:none;padding:10px;">Cancel</a>`:''}</div></form></section>
