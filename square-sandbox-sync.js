...
function createSync({q, ready, enabled=true}) {
  ...
  const prepare=async()=>{
    await ready();
    return schema || (schema=q(`CREATE TABLE IF NOT EXISTS square_sandbox_sync (
  ...
  async function run(id, job, dueOnly=false) {
    await prepare();
    await q(`INSERT INTO square_sandbox_sync(customer_id)
      SELECT customer_id FROM square_sandbox_connections WHERE customer_id=$1
      ON CONFLICT(customer_id) DO NOTHING`,[id]);
    const lease=crypto.randomUUID();
    const claim=await q(`UPDATE square_sandbox_sync SET lease=$2,
    ...
    try {
      await job(lease);
      await q(`UPDATE square_sandbox_sync SET lease=NULL,lease_until=NULL,status='ok',
        last_success=NOW(),next_at=NOW()+INTERVAL '5 minutes' WHERE customer_id=$1 AND lease=$2`,[id,lease]);
      return true;
    } catch(e) {
      await q(`UPDATE square_sandbox_sync SET lease=NULL,lease_until=NULL,status='retry',
        next_at=NOW()+INTERVAL '1 minute' WHERE customer_id=$1 AND lease=$2`,[id,lease]);
      throw e;
    }
  }
  async function status(id){await prepare();return (await q('SELECT status,last_success,last_attempt FROM square_sandbox_sync WHERE customer_id=$1',[id])).rows[0];}
  async function tick(sync) {
    if(running || !enabled)return;
    running=true;
    try {
      await prepare();
      const due=await q(`SELECT c.customer_id FROM square_sandbox_connections c
        LEFT JOIN square_sandbox_sync s ON s.customer_id=c.customer_id
