...
function renderReadinessCenter({title="AI Readiness Center",profile={},readiness,action,backHref="",saved=false,evidenceConnectorHref=""}) {
  ...
  ${evidenceConnectorHref?`<p><a href="${esc(evidenceConnectorHref.replace('/org-connectors/google-ads/','/org-marketing-command-center/'))}" style="color:#1559c7;font-weight:800;">Open Marketing Command Center →</a></p>`:""}
  ...
  <h2 style="margin:26px 0 12px;">Evidence Passport</h2>${evidenceConnectorHref?`<div style="margin-bottom:14px;padding:16px;border:1px solid #b8cff2;border-radius:13px;background:#f4f8ff;"><strong>Connect verified platform evidence</strong><p style="margin:5px 0 11px;color:#52667e;">Observe Google Ads spend, delivery and conversion performance without permitting Vivid to change campaigns.</p><a href="${esc(evidenceConnectorHref)}" style="display:inline-block;padding:10px 13px;border-radius:9px;background:#1559c7;color:#fff;text-decoration:none;font-weight:900;">Open Google Ads Observation →</a></div>`:""}<section style="display:grid;grid-template-columns:repeat(auto-fit,minmax(285px,1fr));gap:13px;">${readiness.evidence.map(item=>`<article style="border:1px solid #dbe5f0;border-radius:14px;background:#fff;padding:17px;"><div style="display:flex;justify-content:space-between;gap:12px;"><strong>${esc(item.label)}</strong><strong style="color:#1559c7;">${item.earned}/${item.weight}</strong></div><div style="height:7px;background:#e8eef8;border-radius:99px;margin:11px 0;overflow:hidden;"><div style="height:100%;width:${item.weight?item.earned/item.weight*100:0}%;background:#1559c7;"></div></div><p style="font-size:13px;color:#52667e;line-height:1.45;margin:0;">${esc(item.proof)}</p>${item.earned<item.weight?`<p style="font-size:12px;color:#8a5a00;margin:9px 0 0;"><strong>To improve:</strong> ${esc(item.fix)}</p>`:""}</article>`).join("")}</section>
