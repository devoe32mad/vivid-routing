...
function configuration(env) {
  const key = Buffer.from(env.SQUARE_SANDBOX_TOKEN_KEY || '', 'base64');
  const redirect = env.SQUARE_SANDBOX_REDIRECT_URL || '';
  const url = new URL(redirect);
  if (key.length !== 32 || !env.SQUARE_SANDBOX_APPLICATION_SECRET ||
      !String(env.SQUARE_SANDBOX_APPLICATION_ID).startsWith('sandbox-') ||
  ...
  return {key, redirect, id: env.SQUARE_SANDBOX_APPLICATION_ID, secret: env.SQUARE_SANDBOX_APPLICATION_SECRET};
}
function install({app, q, requireAdvertiserCustomerManager, env = process.env, fetcher = fetch}) {
  ...
  if (env.SQUARE_SANDBOX_ENABLED !== 'true') return;
  ...
  const wrap = fn => async (req, res) => {
    ...
    try {await ready(); await fn(req, res);} catch (_) {
      ...
      res.status(502).send('Square Sandbox request could not be completed. Please retry or reconnect.');
    }
  };
  const root = id => PATH + '/customers/' + id;
  const csrf = req => equal(req.body?.csrf, req.session.squareSandboxCsrf);
  ...
  app.post(PATH + '/customers/:customerId/connect', owner, wrap(async (req, res) => {
    ...
    await q('DELETE FROM square_sandbox_states WHERE expires_at < NOW()');
    await q('INSERT INTO square_sandbox_states(state_hash,customer_id,expires_at) VALUES($1,$2,$3)',
      [crypto.createHash('sha256').update(state).digest('hex'),id,new Date(Date.now()+600000)]);
    await save(req);
    const url = new URL(BASE + '/oauth2/authorize');
    url.search = new URLSearchParams({client_id:config.id, scope:SCOPES, state, redirect_uri:config.redirect}).toString();
    res.redirect(url.toString());
  }));
  app.get(PATH + '/callback', (req, res, next) => {
    const pending = req.session?.squareSandboxPending;
    ...
    const pending = req.session.squareSandboxPending;
    delete req.session.squareSandboxPending;
    await save(req);
    const consumed = await q('DELETE FROM square_sandbox_states WHERE state_hash=$1 AND customer_id=$2 AND expires_at>NOW() RETURNING customer_id',
  ...
  app.post(PATH + '/customers/:customerId/disconnect', owner, wrap(async (req, res) => {
    ...
    const id = Number(req.params.customerId);
    const result = await q('SELECT token_ciphertext FROM square_sandbox_connections WHERE customer_id=$1', [id]);
    if (result.rows[0]) {
      ...
      await q('DELETE FROM square_sandbox_connections WHERE customer_id=$1', [id]);
    }
    delete req.session.squareSandboxPending;
