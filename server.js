...
app.get("/dashboard", async (req, res) => {
  try {
    ...
    const qrRows = await q(`SELECT qr.id AS qr_id, qr.name AS qr_name, qr.created_at, s.name AS space_name, s.location, s.annual_impressions, s.placement_costFROM qr_codes qr LEFT JOIN spaces s ON s.id = qr.space_id ORDER BY qr.id`);
  ...
  liveDays;
    ...
    res.send(page("Vivid ROI Dashboard", `
      ...
      <div class="wrap">
        ...
        <a class="btn" href="/r/1">Test QR</a><a class="btn secondary" href="/admin">Admin</a><a class="btn secondary" href="/admin/schedule">Schedule Campaigns</a><a class="btn secondary" href="/admin/assign">Assign Campaign</a><a class="btn gold" href="/export/events.csv">Export CSV</a>
...
app.get("/admin", async (req, res) => {
  ...
  res.send(page("Vivid Admin", `
    ...
    <div class="wrap"><a class="btn" href="/dashboard">Dashboard</a><a class="btn secondary" href="/admin/new-location">New Location</a><a class="btn secondary" href="/admin/new-qr">New QR</a><a class="btn secondary" href="/admin/new-campaign">New Campaign</a><a class="btn secondary" href="/admin/new-store">New Store</a><a class="btn secondary" href="/admin/schedule">Schedule Campaigns</a><a class="btn secondary" href="/admin/assign">Assign Campaign</a>
      ...
      <h2>Campaigns</h2><table><tr><th>ID</th><th>Advertiser</th><th>Campaign</th><th>URL</th><th>Avg Value</th><th>Conversion</th></tr>${campaigns.rows.map(c => `<tr><td>${c.id}</td><td>${c.advertiser || ""}</td><td>${c.name || ""}</td><td>${c.campaign_url || ""}</td><td>${money(c.avg_customer_value)}</td><td>${c.conversion_rate || 10}%</td></tr>`).join("")}</table>
...
app.post("/admin/new-location", async (req, res) => {
  try {
    await q(`INSERT INTO spaces (name,location,description,annual_impressions,placement_cost) VALUES ($1,$2,$3,$4,$5)`, [req.body.name, req.body.location, req.body.description, Number(req.body.annual_impressions || 0), Number(req.body.placement_cost || 0)]);
