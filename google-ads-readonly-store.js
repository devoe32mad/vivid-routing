...
const PUBLIC_COLUMNS = "id,customer_id,account_name,currency_code,account_timezone,status,last_error,last_synced_at,last_from::text,last_to::text";
const SAFE_ERRORS = new Set(["authorization","temporary","network","google_access","account","invalid_report","report_too_large"]);
    ...
    async authorize(userId,pending,token,account) {
      ...
      return transaction(async client => {
        ...
        const row = (await client.query(`INSERT INTO google_ads_private_connections
          ...
          account_timezone=EXCLUDED.account_timezone,token_ciphertext=EXCLUDED.token_ciphertext,status='connected',last_error='',updated_at=NOW()
    ...
    async sync(userId,id,range) {
      ...
      const count = await transaction(async client => {
        ...
        try {
          ...
          await client.query("UPDATE google_ads_private_connections SET last_error=$1,status=CASE WHEN $1='authorization' THEN 'attention_required' ELSE status END WHERE id=$2 AND owner_user_id=$3",[failure,id,userId]);
          return 0;
        }
        ...
        await client.query(`UPDATE google_ads_private_connections SET status='connected',last_error='',last_synced_at=NOW(),last_from=$1,last_to=$2,
          currency_code=$3,account_timezone=$4,account_name=$5,updated_at=NOW() WHERE id=$6 AND owner_user_id=$7`,
        [range.from,range.to,account.currencyCode,account.timeZone,String(account.descriptiveName || row.customer_id).slice(0,240),id,userId]);
        return data.length;
      });
...
async function dashboardEvidence(q,userId,range) {
  ...
  } catch(error) { if(error.code === "42P01") return {connections:[],rows:[]}; throw error; }
}
