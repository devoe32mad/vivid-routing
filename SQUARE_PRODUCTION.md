...
4. Customer opens the campaign QR and claims the offer in Vivid. An eligible scan produces one random 64-bit code, formatted `VIVID-ABCD-1234-EF56-7890`. Repeated claims from the same scan return the same code. Claims do not produce conversion events. Claims expire after seven days, capped by the original scan's 30-day attribution window. This is single use per claim, not a one-per-person offer limit.
