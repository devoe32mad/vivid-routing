...
5. The customer claim displays a QR generated inside Vivid (no third-party QR service). A signed-in, authorized Vivid advertiser manager scans it with the phone camera to open the offer directly. Manual code entry remains available at `/customers/ADVERTISER_ID/instore/redeem` under the Square production root. A single active USD checkout location is selected automatically; for multiple locations the cashier explicitly chooses once, with the selection remembered in the session for that customer and connected merchant. The displayed location can be changed. A scanned link never overrides a remembered checkout location and never approves a claim. Vivid checks ownership, location, expiration and prior redemption. No new cashier role is granted automatically.
6. Staff uses one explicit **Confirm eligible items & copy code** button. Its label and adjacent text confirm that staff checked eligibility and will apply the offer. The authenticated, CSRF-protected POST approves the checkout, then the client attempts to copy the exact returned code. Clipboard denial offers manual selection/copy; network or sign-in failures do not show an approved state or copy a code. Without JavaScript the same form submits normally, with the code available for manual copying. Approval reserves a 30-minute payment window (or the claim expiry, if earlier). Repeating approval resumes that same window, never extends it. Staff manually applies the benefit in Square and puts only the complete code in an eligible item's note. A used or expired code cannot be approved again. Abandoned approvals expire; this pilot does not reset or recycle them.
7. Complete one USD card payment for the full Square order. Split tenders, cash, gift cards and partially paid orders are unsupported. Vivid requires the completed order's single card tender to name the exact imported payment and agree on location and total. Square continues to handle items, taxes and payment; the cashier still applies the benefit and pastes the code into the item Note. Scanning uses the phone camera to open Vivid, not Square’s barcode scanner. This release does not inject a code or discount into an existing Square cart.
...
- Only normalized code, eligibility and order-close timestamp are retained from notes; arbitrary notes and card/customer details are discarded. The customer QR contains an HTTPS Vivid cashier URL with the code in its query string; it contains no payment/card data or credentials. Only authenticated authorized staff can look up that URL. Approvals still require explicit POST bodies and CSRF. Existing no-store/no-referrer response headers apply; never log claim URLs. A QR link can remain in device browsing history, but does not grant merchant access or bypass eligibility checks.
...
PGlite-backed tests exercise the actual route/model/sync SQL: scan -> claim -> approval -> completed order -> one native conversion, repeat imports, refund adjustments, tenant/location/CSRF restrictions, frozen offers, expiry, split payments, conflicting codes/references, lease failure, atomic write rollback/recovery and concurrent claim/approval retries. Square responses are simulated. On September 22, 2026 the user completed a live in-store purchase with an approved short claim code. The Square report showed two matched $1 payments (one online, one in store); the campaign overview confirmed two conversions and $2 revenue. The earlier diagnostic sale remained unmatched in the unfiltered report. This validates the existing item-note attribution flow. The new scan-to-cashier and combined confirmation UI still requires a device usability check; automated tests do not establish camera or clipboard behavior on every phone.

If another live acceptance test is explicitly requested, use a fresh scan and the actual newly issued code, validate/approve it through Vivid before payment, then follow the normal itemized Square checkout. Confirm one native conversion on the correct claim action and matching scan/QR evidence. Refresh twice to confirm no duplicate. The earlier diagnostic sale must remain unmatched. No further charge or refund is authorized by these instructions alone.
...
### Checkout automation boundary

Square’s [Point of Sale API limitations](https://developer.squareup.com/docs/pos-api/what-it-does) explicitly exclude itemized sales. Its [mobile web API](https://developer.squareup.com/docs/pos-api/web-technical-reference) starts a payment request; it does not provide an existing itemized cart injection field. The [Terminal API](https://developer.squareup.com/docs/terminal-api/overview) supports a custom POS application paired with Square Terminal hardware. These are different checkout integrations and are not enabled by this UI improvement. Preserve normal Square basket handling and exact approved-code attribution until a deeper supported flow is separately implemented and validated.
