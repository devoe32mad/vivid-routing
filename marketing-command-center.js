...
function renderCommandCenter({title,scope,range,campaigns,squareStatus,googleEvidence=null,googleEnabled=false,googleAutoSync=true,metaEvidence=null,metaEnabled=false,metaAutoSync=true,platform="",campaign="",aiVisible=false}) {
  ...
  ].filter(m=>scope.kind==="advertiser"||!["google_ads","meta"].includes(m[3]));
...
<p class="mcc-coverage">${scope.kind==="enterprise"?"Only this advertiser’s campaigns shared with your organization. Private advertising accounts and merchant sales are excluded.":"All campaigns owned by your signed-in account and reports from your connected platforms. Vivid and Square dates use UTC; Google and Meta dates use each account’s timezone."}</p>
