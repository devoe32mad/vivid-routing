...
function renderCommandCenter({title,scope,range,campaigns,squareStatus,googleEvidence=null,googleEnabled=false,googleAutoSync=true,metaEvidence=null,metaEnabled=false,metaAutoSync=true,linkedinEvidence=null,linkedinEnabled=false,linkedinAutoSync=true,analyticsEvidence=null,analyticsEnabled=false,analyticsAutoSync=true,platform="",campaign="",aiVisible=false}) {
  ...
  const googleRows=googleEvidence?.rows||[],metaRows=metaEvidence?.rows||[],linkedinRows=linkedinEvidence?.rows||[],adRows=[...googleRows,...metaRows,...linkedinRows];
  ...
  const meta=platforms.find(p=>p.id==="meta"),mm=meta?.metrics||[],hasAdData=adRows.length>0;
  const metrics=[
    ["Scans",totals.scans,"Vivid placements","vivid"],
    ["Intent actions",totals.clicks,"Offers, maps & destination actions","vivid"],
    ["Ad impressions",hasAdData?adImpressions.toLocaleString("en-US"):"—","Google + Meta + LinkedIn imported reports","google_ads"],
    ["Ad clicks",hasAdData?adClicks.toLocaleString("en-US"):"—","Google + Meta + LinkedIn imported reports","google_ads"],
    ["Ad spend",groupedMoney(adRows,"cost_micros"),"Google + Meta + LinkedIn · currencies kept separate","google_ads"],
    ["Ad click-through rate",hasAdData&&adImpressions?(100*adClicks/adImpressions).toLocaleString("en-US",{maximumFractionDigits:2})+"%":"—","Combined clicks / combined impressions","google_ads"],
    ...
    ["Google-reported conversions",googleRows.length?googleRows.reduce((s,r)=>s+n(r.conversions),0).toLocaleString("en-US"):"—","Google Ads · may overlap recorded outcomes","google_ads"],
    ["Google-reported value",groupedMoney(googleRows,"conversion_value"),"Google Ads · not verified revenue","google_ads"],
    ["Meta-reported purchases",mm[5]?.[1]||"—","Meta Ads · may overlap recorded outcomes","meta"],
    ["Meta-reported leads",mm[6]?.[1]||"—","Meta Ads · may overlap other lead sources","meta"],
    ["Meta-reported purchase value",mm[7]?.[1]||"—","Meta Ads · not verified revenue","meta"]
  ].filter(m=>(scope.kind==="advertiser"&&scope.privateAdsAllowed!==false)||!["google_ads","meta"].includes(m[3]));
  const roadmap=CONNECTORS.filter(c=>!['vivid','square','google_ads','meta','linkedin'].includes(c.id));
  return `<style>
...
<p class="mcc-coverage">${scope.kind==="enterprise"?"Only this advertiser’s campaigns shared with your organization. Private advertising accounts and merchant sales are excluded.":"Campaigns and reports for the account shown above. Vivid and Square dates use UTC; Google, Meta and LinkedIn dates use each account’s timezone."}</p>
...
<p class="mcc-coverage"><strong>Revenue accounting:</strong> Square matched purchases and value are included in Vivid recorded outcomes and are not added again. Google-, Meta- and LinkedIn-reported outcomes remain separate because they can overlap each other and Vivid records. No combined revenue or conversion total is inferred. A dash means no reporting data is available.</p>
