...
function renderCommandCenter({title,scope,range,campaigns,squareStatus,googleEvidence=null,googleEnabled=false,googleAutoSync=true,metaEvidence=null,metaEnabled=false,metaAutoSync=true,platform="",campaign="",aiVisible=false}) {
  const totals=summarize(campaigns),recs=recommendations(campaigns,scope);
  const platforms=buildPlatforms({scope,range,campaigns,squareStatus,googleEvidence,googleEnabled,googleAutoSync,metaEvidence,metaEnabled,metaAutoSync});
  ...
  const googleRows=googleEvidence?.rows||[],metaRows=metaEvidence?.rows||[],adRows=[...googleRows,...metaRows];
  ...
  const metrics=[
    ...
    ["Ad impressions",hasAdData?adImpressions.toLocaleString("en-US"):"—","Google + Meta imported reports","google_ads"],
    ["Ad clicks",hasAdData?adClicks.toLocaleString("en-US"):"—","Google + Meta imported reports","google_ads"],
    ["Ad spend",groupedMoney(adRows,"cost_micros"),"Google + Meta · currencies kept separate","google_ads"],
  ...
  const roadmap=CONNECTORS.filter(c=>!['vivid','square','google_ads','meta'].includes(c.id));
  return `<style>
...
<p class="mcc-coverage">${scope.kind==="enterprise"?"Only this advertiser’s campaigns shared with your organization. Private advertising accounts and merchant sales are excluded.":"Campaigns and reports for the account shown above. Vivid and Square dates use UTC; Google and Meta dates use each account’s timezone."}</p>
...
<p class="mcc-coverage"><strong>Revenue accounting:</strong> Square matched purchases and value are included in Vivid recorded outcomes and are not added again. Google- and Meta-reported outcomes remain separate because they can overlap each other and Vivid records. No combined revenue or conversion total is inferred. A dash means no reporting data is available.</p>
